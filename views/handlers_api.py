...
@handlers_api.route("/api/mq_handler/create", methods=['post'])
def create_mq_handler():
    try:
        json_data, handlers, api = request.get_json(force=True), [], init_dpapi(request.args)
...
@handlers_api.route("/api/http_handler/create", methods=['post'])
